...
public class PathDrawable extends Drawable {
     ...
     * 默认是fill样式
     */
    public PathDrawable(String pathData, int color) {
     ...
     * 默认是方形样式
     */
    public PathDrawable(String pathData, int color, float strokeWidth, Style style) {
        init(pathData, 32, 32, color, strokeWidth, style, Cap.SQUARE);
    }

    /**
     * Create a new PathDrawable based on XML file
     */
    public PathDrawable(String pathData, int width, int height, int color, float strokeWidth, Style style, Cap capType) {
        init(pathData, width, height, color, strokeWidth, style, capType);
    }

    private void init(String pathData, int width, int height, int color, float strokeWidth, Style style, Cap capType) {
        try {
            init(width, height);
            addLayer(pathData, color, strokeWidth, style, capType);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
        ...
        for (type = parser.next();
            ...
            if (type == XmlPullParser.START_TAG) {
                if (parser.getName().equals("layer")) {
                    ...
                    addLayer(data, strokeColor, strokeWidth, strokeStyleFags, capTypeInt, tag);
//                    Style style;
//                    if (strokeStyleFags == 1) {
//                        style = Style.STROKE;
//                    } else if (strokeStyleFags == 2) {
//                        style = Style.FILL;
//                    } else {
...
//                    layer.mTag = tag;
    ...
    private void addLayer(String pathData, int strokeColor, float strokeWidth, int strokeStyleFags, int capTypeInt, String tag) {
        Style style;
        if (strokeStyleFags == 1) {
            style = Style.STROKE;
        } else if (strokeStyleFags == 2) {
        ...
        Layer layer = addLayer(pathData, strokeColor, strokeWidth, style, capType);
        layer.mTag = tag;
    }
